...
BEGIN
    ...
    BEGIN
        ...
        EXEC sp_executesql @sql;
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Business Process and_or Operational impact')
    BEGIN
        SET @sql = N'EXEC sp_rename ''' + REPLACE(@tbl_iwork,'''','''''') + N'.[Business Process and_or Operational impact]'', ''Business Process and/or Operational Impact'', ''COLUMN''';
        EXEC sp_executesql @sql;
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Business Process and_or Operational Impact')
    BEGIN
        ...
        EXEC sp_executesql @sql;
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'If ''Yes''_ note the laws or regulatory bodies_')
    BEGIN
        DECLARE @ifyes_new NVARCHAR(256) = N'If ''Yes'', note the laws or regulatory bodies:';
        DECLARE @ifyes_obj NVARCHAR(776) = @tbl_iwork + N'.[If ''Yes''_ note the laws or regulatory bodies_]';
    ...
    BEGIN
        ...
        EXEC sp_executesql @sql;
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'L1 Net Recurring Benefits $_annualized-Soft')
    BEGIN
        SET @sql = N'EXEC sp_rename ''' + REPLACE(@tbl_iwork,'''','''''') + N'.[L1 Net Recurring Benefits $_annualized-Soft]'', ''Net Recurring Benefits -Soft'', ''COLUMN''';
        EXEC sp_executesql @sql;
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'L2 One-time Benefit $-Soft')
    BEGIN
        SET @sql = N'EXEC sp_rename ''' + REPLACE(@tbl_iwork,'''','''''') + N'.[L2 One-time Benefit $-Soft]'', ''L2 One-time Benefit ($)-Soft'', ''COLUMN''';
        EXEC sp_executesql @sql;
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Lifecycle Status')
    BEGIN
    ...
    BEGIN
        DECLARE @risk_col_new  NVARCHAR(256) = N'*Risk Subtype (Does this request address a risk or requirement to one of the following?)';
        DECLARE @risk_col_obj  NVARCHAR(776) = @tbl_iwork + N'.[Demand SubType]';
        EXEC sp_rename @objname = @risk_col_obj, @newname = @risk_col_new, @objtype = 'COLUMN';
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Risk Subtype Does this request address a risk or requirement to one of the following?')
    BEGIN
        DECLARE @risk_col_new2  NVARCHAR(256) = N'*Risk Subtype (Does this request address a risk or requirement to one of the following?)';
        DECLARE @risk_col_obj2  NVARCHAR(776) = @tbl_iwork + N'.[Risk Subtype Does this request address a risk or requirement to one of the following?]';
        EXEC sp_rename @objname = @risk_col_obj2, @newname = @risk_col_new2, @objtype = 'COLUMN';
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Demand Domain or Portfolio')
    BEGIN
