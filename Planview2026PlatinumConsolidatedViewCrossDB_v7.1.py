...
def load_config(config_path=None):
    ...
    COLUMN_RENAME        = {
        "Initiatives":  raw_rename.get("Initiatives",  {}),
        "Epics":        raw_rename.get("Epics",        {}),
        "Value_Bundles": raw_rename.get("Value_Bundles", {}),
        "Tasks":        raw_rename.get("Tasks",        {}),
    }
...
def apply_column_rename(df, entity):
    ...
    Runs before loading into SQL so input tables have the column names
    that SP2 expects. e.g. view may call a field
    'NRB (Exit Rate) 2028 Latest Estimate.' but SP2 expects
    'NRB (Exit Rate) 2028 (Latest Estimate).'
    """
    ...
    if actual_rename:
        ...
        for old_name, new_name in actual_rename.items():
            log(f"  Column rename [{entity}]: '{old_name}' → '{new_name}'", 2)
    missing = [k for k in rename_map if k not in df.columns]
    for m in missing:
        log(f"  WARNING: column_rename source not found in [{entity}]: '{m}'", 2)
    return df
