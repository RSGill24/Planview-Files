...
def load_sp1_to_sql(conn, ts, df_init_sp1, df_epic_sp1):
    """
    Saves SP1 result sets directly into SQL as separate staging tables:
      output_{ts}.SP1_Initiatives
      output_{ts}.SP1_Epics
    No Python DataFrame merging. No column list. No positional mismatch.
    SP2 structural columns (Stage, EVR, Flow Type, Strategy Seq ID) are
    never touched — SP1 result is kept completely separate from output_{ts}.
    """
    log("Saving SP1 results directly to SQL staging tables...", 1)
    output_schema = f"output_{ts}"

    def save(df, table, label):
        if df.empty:
            log(f"  {label}: empty — skipping", 2)
            return
        cursor = conn.cursor()
        full_table = f"[{output_schema}].[{table}]"
        cursor.execute(f"""
            IF OBJECT_ID('{output_schema}.{table}') IS NOT NULL
                DROP TABLE {full_table}
        """)
        # Build CREATE TABLE using exact column names from SP1 result
        def safe_col(c):
            return str(c).replace(']','').replace("'","").strip()[:120]

        col_defs = ", ".join(f"[{safe_col(c)}] NVARCHAR(MAX)" for c in df.columns)
        cursor.execute(f"CREATE TABLE {full_table} ({col_defs})")

        col_names    = ", ".join(f"[{safe_col(c)}]" for c in df.columns)
        placeholders = ", ".join("?" for _ in df.columns)
        ins = f"INSERT INTO {full_table} ({col_names}) VALUES ({placeholders})"
        ...
        def val(v):
            ...
            s = str(v).strip()
            if s.upper() in ('NULL','NONE','NAN','NAT',''): return None
            # Line breaks preserved - no stripping of newline chars
            return s.replace('|',' ').replace('"',"'")

        batch = []
        for _, row in df.iterrows():
            batch.append(tuple(val(v) for v in row))
            if len(batch) == 500:
        ...
        if batch:
            cursor.executemany(ins, batch)
        conn.commit()
        log(f"  {label}: {len(df):,} rows, {len(df.columns)} columns saved to {full_table}", 2)

    save(df_init_sp1, "SP1_Initiatives", "SP1 Initiatives")
    save(df_epic_sp1, "SP1_Epics",       "SP1 Epics")


def build_final_output(conn, ts):
    """
    Assembles the final unified output by JOINing:
      output_{ts}.Initiatives  — SP2 columns + New SP columns
      output_{ts}.SP1_Initiatives — SP1 mapped/renamed columns
    in SQL. No Python DataFrame merging. SQL handles all column assembly.
    Returns two DataFrames: final Initiatives and final Epics.
    """
    log("Building final output via SQL JOIN...", 1)
    output_schema = f"output_{ts}"
    cursor = conn.cursor()

    # ── Initiatives: get all columns from both tables ──────────────────────
    # SP1 columns — everything except columns SP2 already owns
    sp2_owned = {
        'Strategy Seq ID','Index_ID',
        'Flow Type','Flow Type - Step 1',
    ...
    # Get SP1_Initiatives column list
    cursor.execute(f"""
        SELECT name FROM sys.columns
        WHERE object_id = OBJECT_ID('[{output_schema}].[SP1_Initiatives]')
        ORDER BY column_id
    """)
    sp1_init_cols = [r[0] for r in cursor.fetchall()]

    # SP1 columns to add = SP1 columns not already in SP2 output
    # Get SP2 output column list
    cursor.execute(f"""
        SELECT name FROM sys.columns
        WHERE object_id = OBJECT_ID('[{output_schema}].[Initiatives]')
        ORDER BY column_id
    """)
    sp2_init_cols = {r[0] for r in cursor.fetchall()}

    # SP1 columns to bring in: all SP1 cols except SP2-owned and join key
    sp1_add_cols = [
        c for c in sp1_init_cols
        if c not in sp2_init_cols and c not in sp2_owned
        and c.upper() not in ('RUN_ID','LOAD_TIMESTAMP','SAVE_TIMESTAMP')
    ]

    log(f"  Initiatives: SP2 has {len(sp2_init_cols)} cols, adding {len(sp1_add_cols)} SP1 cols", 2)

    if sp1_add_cols:
        sp1_select = ", ".join(f"sp1.[{c}]" for c in sp1_add_cols)
        init_sql = f"""
            SELECT sp2.*, {sp1_select}
            FROM [{output_schema}].[Initiatives] sp2
            LEFT JOIN [{output_schema}].[SP1_Initiatives] sp1
                ON sp2.[Strategy Seq ID] = sp1.[Strategy Seq ID]
            ORDER BY sp2.[Index_ID]
        """
    else:
        init_sql = f"""
            SELECT * FROM [{output_schema}].[Initiatives]
            ORDER BY [Index_ID]
        """
    ...
    log(f"  Initiatives final: {len(df_init):,} rows | {len(df_init.columns)} cols", 2)

    # ── Epics: same pattern ────────────────────────────────────────────────
    cursor.execute(f"""
        SELECT name FROM sys.columns
        WHERE object_id = OBJECT_ID('[{output_schema}].[SP1_Epics]')
        ORDER BY column_id
    """)
    sp1_epic_cols = [r[0] for r in cursor.fetchall()]

    cursor.execute(f"""
        SELECT name FROM sys.columns
        WHERE object_id = OBJECT_ID('[{output_schema}].[Epics]')
        ORDER BY column_id
    """)
    sp2_epic_cols = {r[0] for r in cursor.fetchall()}

    sp1_add_epic = [
        c for c in sp1_epic_cols
        if c not in sp2_epic_cols
        and c.upper() not in ('RUN_ID','LOAD_TIMESTAMP','SAVE_TIMESTAMP')
    ]

    log(f"  Epics: SP2 has {len(sp2_epic_cols)} cols, adding {len(sp1_add_epic)} SP1 cols", 2)

    if sp1_add_epic:
        sp1_epic_select = ", ".join(f"sp1.[{c}]" for c in sp1_add_epic)
        epic_sql = f"""
            SELECT sp2.*, {sp1_epic_select}
            FROM [{output_schema}].[Epics] sp2
            LEFT JOIN [{output_schema}].[SP1_Epics] sp1
                ON sp2.[Sequence ID] = sp1.[Sequence ID]
            ORDER BY sp2.[Index_ID]
        """
    else:
        epic_sql = f"""
            SELECT * FROM [{output_schema}].[Epics]
            ORDER BY [Index_ID]
        """

    cursor.execute(epic_sql)
    cols = [d[0] for d in cursor.description]
    df_epic = pd.DataFrame.from_records(
        [tuple(r) for r in cursor.fetchall()], columns=cols
    )
    log(f"  Epics final: {len(df_epic):,} rows | {len(df_epic.columns)} cols", 2)

    return df_init, df_epic
...
def main():
    ...
    # ── Step 4b — Save SP1 results to separate SQL tables ─────
    load_sp1_to_sql(conn, ts, df_init_sp1, df_epic_sp1)

    # ── Step 5 — New SP: PQ transformation blocks ─────────────
    # New SP mutates output_{ts} directly — result sets not used for final output
    run_new_sp(conn, ts)

    # ── Step 5b — Build final output via SQL JOIN ───────────────
    # Joins output_{ts}.Initiatives (SP2+New SP) with SP1_Initiatives
    # Pure SQL JOIN — no Python DataFrame merging, no column conflicts
    df_init_final, df_epic_final = build_final_output(conn, ts)
