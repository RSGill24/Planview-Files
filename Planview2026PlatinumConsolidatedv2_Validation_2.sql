...
-- ============================================================
-- Create permanent Validation_Results table if not exists
-- Each run appends a new batch identified by Run_ID + Run_Timestamp
-- ============================================================

IF NOT EXISTS (
    SELECT 1 FROM sys.schemas WHERE name = 'PlanviewMigration_Output_v2'
)
    EXEC('CREATE SCHEMA PlanviewMigration_Output_v2');

IF OBJECT_ID('PlanviewMigration_Output_v2.Validation_Results', 'U') IS NULL
BEGIN
    CREATE TABLE PlanviewMigration_Output_v2.Validation_Results (
        Run_ID          INT,
...
END
ELSE
    PRINT '  Validation_Results table exists — new run will be appended.';

-- Get next Run_ID (increment from last run)
DECLARE @run_id INT;
SELECT @run_id = ISNULL(MAX(Run_ID), 0) + 1
FROM PlanviewMigration_Output_v2.Validation_Results;

DECLARE @run_ts DATETIME = GETDATE();
PRINT '  Run_ID: ' + CAST(@run_id AS VARCHAR) + ' | Timestamp: ' + CONVERT(VARCHAR, @run_ts, 120);

-- Temp table to collect results for this run before inserting
...
-- ── Save all results to permanent table ──────────────────────
INSERT INTO PlanviewMigration_Output_v2.Validation_Results (
    Run_ID, Run_Timestamp, Check_ID, Entity, Tracker_ID,
    Check_Name, Expected, Actual, Result
)
SELECT
    @run_id, @run_ts, Check_ID, Entity, Tracker_ID,
    Check_Name, Expected, Actual, Result
FROM #validation_results;

PRINT '  Results saved to PlanviewMigration_Output_v2.Validation_Results';
PRINT '  Run_ID: ' + CAST(@run_id AS VARCHAR) + ' | Rows inserted: ' + CAST(@@ROWCOUNT AS VARCHAR);
...
-- ── Run history summary — all runs ───────────────────────────
PRINT '';
PRINT '-- VALIDATION RUN HISTORY:';
SELECT
    Run_ID,
    Run_Timestamp,
    SUM(CASE WHEN Result = 'PASS' THEN 1 ELSE 0 END)  AS Pass_Count,
    SUM(CASE WHEN Result = 'FAIL' THEN 1 ELSE 0 END)  AS Fail_Count,
    SUM(CASE WHEN Result = 'INFO' THEN 1 ELSE 0 END)  AS Info_Count
FROM PlanviewMigration_Output_v2.Validation_Results
