...
def save_to_output_schema(conn, results, run_ts):
    ...
    for table_name, df in results.items():
        ...
        # Use _sql_type() to preserve source column types
        ...
        for col in df_c.columns:
            col_dtype = df_c[col].dtype
            if pd.api.types.is_float_dtype(col_dtype) or pd.api.types.is_integer_dtype(col_dtype):
                # For numeric columns: NaN/Inf → None; valid numerics stay as-is
                ...
                # For datetime columns: NaT → None
                df_c[col] = df_c[col].apply(
                    lambda v: None if pd.isnull(v) else v
                )
            else:
                # For string/object columns: apply safe_val (NaN → None, strip etc.)
