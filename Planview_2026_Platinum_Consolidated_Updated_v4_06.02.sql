...
BEGIN
    ...
    BEGIN
        ...
        EXEC sp_executesql @sql;
    END
    -- LIKE-based rename: Business Process and/or Operational Impact
    DECLARE @col_bproc NVARCHAR(256);
    SELECT @col_bproc = name FROM sys.columns 
    WHERE object_id = OBJECT_ID(@tbl_iwork) AND name LIKE 'Business Process%Operational%';
    IF @col_bproc IS NOT NULL AND @col_bproc <> 'Business Process and/or Operational Impact'
    BEGIN
        DECLARE @bproc_obj NVARCHAR(776) = @tbl_iwork + N'.[' + @col_bproc + N']';
        EXEC sp_rename @objname = @bproc_obj, @newname = N'Business Process and/or Operational Impact', @objtype = 'COLUMN';
    END

    -- LIKE-based rename: If 'Yes', note the laws or regulatory bodies:
    DECLARE @col_ifyes NVARCHAR(256);
    SELECT @col_ifyes = name FROM sys.columns 
    WHERE object_id = OBJECT_ID(@tbl_iwork) AND name LIKE '%Yes%note the laws%';
    IF @col_ifyes IS NOT NULL AND @col_ifyes <> 'If ''Yes'', note the laws or regulatory bodies:'
    BEGIN
        DECLARE @ifyes_new NVARCHAR(256) = N'If ''Yes'', note the laws or regulatory bodies:';
        DECLARE @ifyes_obj NVARCHAR(776) = @tbl_iwork + N'.[' + @col_ifyes + N']';
        EXEC sp_rename @objname = @ifyes_obj, @newname = @ifyes_new, @objtype = 'COLUMN';
    END

    -- LIKE-based rename: L1 Net Recurring Benefits -> Net Recurring Benefits -Soft
    DECLARE @col_l1 NVARCHAR(256);
    SELECT @col_l1 = name FROM sys.columns 
    WHERE object_id = OBJECT_ID(@tbl_iwork) AND name LIKE 'L1 Net Recurring Benefits%';
    IF @col_l1 IS NOT NULL
    BEGIN
        DECLARE @l1_obj NVARCHAR(776) = @tbl_iwork + N'.[' + @col_l1 + N']';
        EXEC sp_rename @objname = @l1_obj, @newname = N'Net Recurring Benefits -Soft', @objtype = 'COLUMN';
    END

    -- LIKE-based rename: L2 One-time Benefit -> L2 One-time Benefit ($)-Soft
    DECLARE @col_l2 NVARCHAR(256);
    SELECT @col_l2 = name FROM sys.columns 
    WHERE object_id = OBJECT_ID(@tbl_iwork) AND name LIKE 'L2 One-time Benefit%';
    IF @col_l2 IS NOT NULL AND @col_l2 <> 'L2 One-time Benefit ($)-Soft'
    BEGIN
        DECLARE @l2_obj NVARCHAR(776) = @tbl_iwork + N'.[' + @col_l2 + N']';
        EXEC sp_rename @objname = @l2_obj, @newname = N'L2 One-time Benefit ($)-Soft', @objtype = 'COLUMN';
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Lifecycle Status')
    BEGIN
    ...
    BEGIN
        ...
        EXEC sp_executesql @sql;
    END
    -- LIKE-based rename: Risk Subtype (full target name with brackets)
    DECLARE @col_risk NVARCHAR(256);
    SELECT @col_risk = name FROM sys.columns 
    WHERE object_id = OBJECT_ID(@tbl_iwork) AND name LIKE '%Risk Subtype%';
    IF @col_risk IS NOT NULL AND @col_risk <> 'Risk Subtype (Does this request address a risk or requirement to one of the following?)'
    BEGIN
        DECLARE @risk_new  NVARCHAR(256) = N'*Risk Subtype (Does this request address a risk or requirement to one of the following?)';
        DECLARE @risk_obj  NVARCHAR(776) = @tbl_iwork + N'.[' + @col_risk + N']';
        EXEC sp_rename @objname = @risk_obj, @newname = @risk_new, @objtype = 'COLUMN';
    END
    IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@tbl_iwork) AND name = 'Demand Domain or Portfolio')
    BEGIN
