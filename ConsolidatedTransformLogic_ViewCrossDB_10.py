...
def align_output_types(conn, version):
    ...
    for out_table, view_name in view_map.items():
        ...
        for col in out_cols:
            ...
            # Skip datetime and numeric types — data stored as string
            # may not convert cleanly (e.g. date codes like '22007').
            # Only alter nvarchar/varchar length restrictions which are safe.
            safe_to_alter = sql_type.lower().startswith(('nvarchar(', 'varchar(', 'nchar(', 'char('))
            if not safe_to_alter:
                continue
            try:
                cursor.execute(f"""
...
def save_to_output_schema(conn, results, run_ts):
    ...
    for table_name, df in results.items():
        ...
        # Convert every value to string or None before insert.
        # This is the only guaranteed way to avoid pyodbc float/decimal
        # type mismatch errors regardless of what dtype pyodbc returns.
        # align_output_types() will restore correct SQL types after insert.
        def _to_str_or_none(v):
            ...
            try:
                ...
                pass
            s = str(v).strip()
            return None if s in ('', 'nan', 'NaN', 'None', 'NaT',
                                  'inf', '-inf', '<NA>') else s

        df_c = df_out.copy()
        rows = [
            tuple(_to_str_or_none(v) for v in row)
            for row in df_c.itertuples(index=False, name=None)
        ]
...
def write_excel(results, frames, run_ts):
    ...
    # No mapping logic in this SP — MAPPED_COLS is empty for all tables
    # _Original columns and amber highlighting only apply when mappings run
    MAPPED_COLS = {
        "Initiatives":   [],
        "Epics":         [],
        "Value_Bundles": [],
