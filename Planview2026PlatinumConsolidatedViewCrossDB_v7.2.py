...
def apply_column_rename(df, entity):
    ...
    Strips whitespace from column names and config keys before comparing
    to avoid invisible mismatch. Logs every config key vs actual columns
    ...
    # Build a stripped version of actual columns for comparison
    # Map stripped_name → original_name so we can rename correctly
    stripped_to_original = {c.strip(): c for c in df.columns}

    actual_rename = {}   # original_col_name → new_name
    for cfg_key, new_name in rename_map.items():
        cfg_key_stripped = cfg_key.strip()
        if cfg_key_stripped in stripped_to_original:
            orig_col = stripped_to_original[cfg_key_stripped]
            actual_rename[orig_col] = new_name
        else:
            # Log mismatch — show config key vs closest actual column names
            close = [c for c in df.columns
                     if cfg_key_stripped[:10].lower() in c.lower()]
            log(f"  WARNING [{entity}]: config rename key not found: '{cfg_key}'", 2)
            if close:
                log(f"    Closest actual columns: {close[:3]}", 2)
            else:
                log(f"    No similar column found in view", 2)

    if actual_rename:
        df = df.rename(columns=actual_rename)
        for orig, new in actual_rename.items():
            log(f"  Column rename [{entity}]: '{orig}' → '{new}'", 2)
    else:
        log(f"  WARNING [{entity}]: column_rename defined but NO renames applied", 2)
        log(f"    Config keys : {list(rename_map.keys())}", 2)
        log(f"    View columns: {list(df.columns[:10])}", 2)
