...
def read_from_view(conn, view_name):
    ...
    try:
        ...
        # ── Fetch data ────────────────────────────────────────────────────────
        # Use a fresh cursor with no parameter scanning to avoid binding errors
        # on views that have column names pyodbc might misinterpret
        try:
            cursor.execute(f"SELECT * FROM {full_name}")
        except Exception as fetch_e:
            # If direct SELECT fails try wrapping in a stored procedure call
        ...
        # ── Build type_map from cursor description type codes ─────────────────
        # pyodbc type_code maps: str→nvarchar, Decimal→decimal, datetime→datetime, int→bigint
        import decimal, datetime as dt_mod
        type_map = {}
        try:
            for col_desc in col_descriptions:
                col_name  = col_desc[0]
                type_code = col_desc[1]  # Python type object
...
def save_output_to_sql(conn, df, schema_name, table_name, ts, type_map=None):
    ...
    # Use col_type_map — both _sql_col stripped and original keys
    col_type_map = {}
    if type_map:
