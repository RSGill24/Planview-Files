        ...
        try:
            if VIEW_DATABASE:
                meta_sql = f"""
                    ...
                    WHERE o.name = '{view_name}' AND s.name = '{VIEW_SCHEMA}'
                ...
                meta_sql = f"""
                    ...
                    WHERE c.object_id = OBJECT_ID('{VIEW_SCHEMA}.{view_name}')
                    ORDER BY c.column_id
                """
...
def load_to_sql(conn, df, schema_name, table_name, ts, type_map=None):
    ...
    df_c = df.copy()
    # Force all columns to object dtype first — prevents pandas from
    # bypassing apply() for typed columns (datetime, int, float etc.)
    for col in df_c.columns:
        df_c[col] = df_c[col].astype(object)

    import math
    def safe_str_val(v):
        """Convert all values to string for input schema — SP works on nvarchar."""
        if v is None:
            return None
        try:
            import pandas as pd
        ...
        if isinstance(v, float) and (math.isnan(v) or math.isinf(v)):
            return None
        s = str(v).strip()
        if s.upper() in ('NULL', 'NAT', 'NONE', 'NAN') or s == '':
            return None
        return (s.replace("\n", " ").replace("\r", " ")
                 .replace("|", " ").replace('"', "'"))

    for col in df_c.columns:
        df_c[col] = df_c[col].apply(safe_str_val)

    rows = [tuple(r) for r in df_c.values.tolist()]
...
def save_output_to_sql(conn, df, schema_name, table_name, ts, type_map=None):
    ...
    # Use type_map from view metadata for correct SQL types in output table
    # Fall back to nvarchar(MAX) if not available
    col_defs = ", ".join([
        f"[{c}] {type_map.get(c, 'nvarchar(MAX)') if type_map else 'nvarchar(MAX)'}"
        for c in df.columns
    ])
    cursor.execute(f"""
        CREATE TABLE [{schema_name}].[{table_name}] (
    ...
    def safe_val(v, col):
        """Cast value to the correct type for the output column."""
        if v is None:
            return None
        if isinstance(v, float) and (math.isnan(v) or math.isinf(v)):
            return None
        try:
            ...
            pass
        # Get target SQL type from type_map
        sql_t = (type_map or {}).get(col, 'nvarchar(MAX)') if type_map else 'nvarchar(MAX)'
        if 'decimal' in sql_t or 'numeric' in sql_t or 'float' in sql_t:
            try:
                return float(v) if str(v).strip() not in ('', 'NULL', 'None') else None
            except (ValueError, TypeError):
                return None
        elif sql_t in ('bigint', 'int'):
            try:
                return int(float(v)) if str(v).strip() not in ('', 'NULL', 'None') else None
            except (ValueError, TypeError):
                return None
        elif sql_t == 'datetime':
            # Pass through datetime objects, return None for blank strings
            if isinstance(v, str):
                s = v.strip()
            ...
            return str(v).strip() if str(v).strip() not in ('', 'NULL', 'None') else None
...
def create_output_schema_and_save(conn, df_inits, df_epics, df_sba, df_tasks, stem, ts,
    ...
    save_output_to_sql(conn, df_inits, schema, f"Initiatives_{v}", ts, type_map_init)
    save_output_to_sql(conn, df_epics, schema, f"Epics_{v}",       ts, type_map_epic)
    save_output_to_sql(conn, df_sba,   schema, f"SBA_{v}",         ts, type_map_sba)
    save_output_to_sql(conn, df_tasks, schema, f"Tasks_{v}",       ts, type_map_task)
...
def main():
    ...
    # ── Build renamed type_maps to account for SP sp_rename column changes ────
    # SP renames: Lifecycle Status→Status, Demand SubType→Risk Subtype,
    #             Is this confidential?→Does this require Limited Visibility?,
    #             Governance Level→Level of Governance
    # type_map keys use original view column names — update keys to match output names
    init_rename_map = {
        'Lifecycle Status':      'Status',
        'Demand SubType':        'Risk Subtype',
        'Is this confidential?': 'Does this require Limited Visibility?',
        'Is this confidential':  'Does this require Limited Visibility?',
    }
    epic_rename_map = {
    ...
    def apply_rename_to_type_map(tmap, rename_map):
        if not tmap:
            return tmap
        result = {}
        for col, sql_type in tmap.items():
            new_col = rename_map.get(col, col)
            result[new_col] = sql_type
        return result

    type_map_init_out = apply_rename_to_type_map(type_map_init, init_rename_map)
    type_map_epic_out = apply_rename_to_type_map(type_map_epic, epic_rename_map)
    ...
    output_schema = create_output_schema_and_save(
        ...
        type_map_init_out, type_map_epic_out, type_map_sba, type_map_task)
