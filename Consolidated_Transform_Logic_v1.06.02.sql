...
BEGIN
    ...
    SET [Stage] =
        CASE
            ...
            WHEN LTRIM(RTRIM(ISNULL([Stage],''''))) IN (''A: L0'',''B: SL1'')
                THEN NULL
            ...
            WHEN LTRIM(RTRIM(ISNULL([Demand Type],'''')))
             ...
             AND COALESCE(NULLIF(LTRIM(RTRIM([Flow Type])),''''),
                          NULLIF(LTRIM(RTRIM([Flow Type - Step 1])),''''))
            ...
            WHEN LTRIM(RTRIM(ISNULL([Demand Type],'''')))
                 IN (''Business w/ Tech'',''Business w/ Tech Initiative'')
             AND COALESCE(NULLIF(LTRIM(RTRIM([Flow Type])),''''),
                          NULLIF(LTRIM(RTRIM([Flow Type - Step 1])),'''')) LIKE ''%ERROR%''
            ...
            -- All other Demand Types with no Stage → NULL
            ELSE NULL
        END
