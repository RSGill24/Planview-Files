...
def apply_column_rename(df, entity):
    ...
    Uses case-insensitive + whitespace-stripped matching so minor
    differences between config keys and actual view column names
    (trailing spaces, different casing) do not prevent the rename.
    Logs every attempt so mismatches are visible in the run log.
    """
    rename_map = COLUMN_RENAME.get(entity, {})
    if not rename_map:
        return df

    # Normalise: lowercase + strip whitespace for comparison only
    # Keep original name for the actual rename operation
    def norm(s):
    ...
    actual_rename = {}
    for cfg_key, new_name in rename_map.items():
        norm_key = norm(cfg_key)
        if norm_key in norm_to_original:
            orig_col = norm_to_original[norm_key]
            actual_rename[orig_col] = new_name.strip()
            log(f"  Column rename [{entity}]: '{orig_col}' → '{new_name.strip()}'", 2)
        else:
            # Show what is in the config vs what is in the DataFrame
            close = [c for c in df.columns
                     if norm(cfg_key)[:12] in norm(c) or norm(c)[:12] in norm(cfg_key)]
            log(f"  WARNING [{entity}]: rename key not matched: '{cfg_key}'", 2)
            if close:
                log(f"    Closest view columns: {close[:5]}", 2)
            else:
                # Print all view columns so user can identify correct name
                log(f"    View columns (first 20): {list(df.columns[:20])}", 2)

    if not actual_rename and rename_map:
        log(f"  WARNING [{entity}]: column_rename in config but NOTHING matched.", 2)
        log(f"    Config keys : {list(rename_map.keys())}", 2)
        log(f"    View columns: {list(df.columns[:20])}", 2)
...
def create_input_schema_and_load(conn, df_inits, df_epics, df_sba, df_tasks, ts,
    ...
    # SP2 expects Initiatives / Epics / Value_Bundles (no stem prefix)
    # SP1 expects {stem}_Initiatives / {stem}_Epics / {stem}_Tasks
    # Load both naming conventions for each entity
    df_inits = apply_column_rename(df_inits, "Initiatives")
    df_epics = apply_column_rename(df_epics, "Epics")
    df_sba   = apply_column_rename(df_sba,   "Value_Bundles") if not df_sba.empty else df_sba
    df_tasks = apply_column_rename(df_tasks, "Tasks")         if not df_tasks.empty else df_tasks
    ...
    if not df_sba.empty:
        ...
        log(f"  SBA view not configured — creating empty placeholder tables", 1)
        for tbl in [tbl_sba, "Value_Bundles"]:
            cursor = conn.cursor()
            cursor.execute(f"""
                IF OBJECT_ID('[{schema}].[{tbl}]') IS NOT NULL
                    DROP TABLE [{schema}].[{tbl}]
            """)
            cursor.execute(f"""
                CREATE TABLE [{schema}].[{tbl}] ([_placeholder] nvarchar(10) NULL)
            """)
            conn.commit()
    ...
    dedup_input_table(conn, schema, tbl_init,       'Strategy Seq ID', f'{stem}_Initiatives')
    dedup_input_table(conn, schema, "Epics",        'Sequence ID',     'Epics')
    dedup_input_table(conn, schema, tbl_epic,       'Sequence ID',     f'{stem}_Epics')
...
# ── STEP 3: Call SP2 — Consolidated_Transform_Logic ──────────
def run_sp2(conn, ts):
    """
