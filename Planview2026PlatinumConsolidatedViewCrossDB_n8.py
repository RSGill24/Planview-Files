...
def save_output_to_sql(conn, df, schema_name, table_name, ts, type_map=None):
    ...
    def to_typed_val(v, col):
        ...
        sql_t = (type_map or {}).get(col, 'nvarchar(MAX)')
        ...
        if 'decimal' in sql_t or 'numeric' in sql_t or 'float' in sql_t:
            ...
            if isinstance(v, (dt.datetime, dt.date)):
                # Check SQL datetime range: 1753-01-01 to 9999-12-31
                try:
                    if isinstance(v, dt.date) and not isinstance(v, dt.datetime):
                        v = dt.datetime(v.year, v.month, v.day)
                    if v.year < 1753 or v.year > 9999:
                        return None
                    return v
                except Exception:
                    return None
            ...
            try:
                ...
                if parsed.year < 1753 or parsed.year > 9999:
                    return None
                return parsed
            except Exception:
                try:
                    parsed = dt.datetime.strptime(s[:19], '%Y-%m-%d %H:%M:%S')
                    if parsed.year < 1753 or parsed.year > 9999:
                        return None
                    return parsed
                except Exception:
