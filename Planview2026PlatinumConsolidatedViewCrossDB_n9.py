...
def save_output_to_sql(conn, df, schema_name, table_name, ts, type_map=None):
    ...
    # Apply _sql_col() to type_map keys to match the column names after stripping
    col_type_map = {}
    if type_map:
        for orig_col, sql_type in type_map.items():
            col_type_map[_sql_col(orig_col)] = sql_type
            col_type_map[orig_col] = sql_type  # also keep original as fallback
    ...
    def to_typed_val(v, col):
        """Cast value to correct Python type matching the output SQL column type."""
        # First handle nulls
        if v is None:
            return None
        ...
        if 'decimal' in sql_t or 'numeric' in sql_t or 'float' in sql_t:
            ...
            SQL_DT_MIN = dt.datetime(1753, 1, 1)
            SQL_DT_MAX = dt.datetime(9999, 12, 31)

            def parse_and_validate(d):
                try:
            ...
            if isinstance(v, (dt.datetime, dt.date)):
                return parse_and_validate(v)

            s = str(v).strip()
            if not s or s.upper() in ('NULL', 'NONE', 'NAT', 'NAN'):
                return None
            # Try common datetime string formats
            for fmt in ('%Y-%m-%d %H:%M:%S', '%Y-%m-%d %H:%M:%S.%f',
                        '%Y-%m-%d', '%d/%m/%Y', '%m/%d/%Y'):
                try:
                    return parse_and_validate(dt.datetime.strptime(s[:26], fmt))
                except Exception:
                    continue
            return None
        else:
