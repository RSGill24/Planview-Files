...
def load_config(config_path=None):
    ...
    if config_path is None:
        config_path = Path(__file__).parent / "ConsolidatedTransformLogic_ViewCrossDB_config.json"
...
def _sql_col(name):
    ...
    '?' IS valid inside brackets in SQL Server — do NOT strip it.
    Strips/replaces characters that break bracket-delimited identifiers.
    """
    clean = (str(name)
...
def read_input():
    log_step("1/7", "Reading input Excel file...")
    path = Path(INPUT_FILE)
    if not path.exists():
        log(f"ERROR: File not found: {path}", 1); sys.exit(1)

    frames = {}
    for table_name, sheet_name in INPUT_SHEETS.items():
        try:
            # Row 1 = system codes, Row 2 = partial names, Row 3 = human names
            ...
            row3 = [str(v).strip() if str(v).strip() not in ('', 'nan') else None
                    for v in raw.iloc[2]]
            # Prefer row 3; fall back to row 2; positional name as last resort
            merged_cols = [_sql_col(r3 or r2 or f"Col_{i}")
            ...
            # Remove system internal column
            if "Work ($Plan) ID" in df.columns:
                df.drop(columns=["Work ($Plan) ID"], inplace=True)

            frames[table_name] = df
            log(f"  {table_name:<15} sheet='{sheet_name}'  "
                f"rows={len(df):,}  cols={len(df.columns)}", 1)

        except Exception as e:
            log(f"  ERROR reading sheet '{sheet_name}': {e}", 1); sys.exit(1)
...
def call_stored_procedure(conn, run_ts):
    ...
    for i, table_name in enumerate(table_order):
        if cursor.description:
            ...
            results[table_name] = pd.DataFrame.from_records(
                [tuple(r) for r in rows], columns=cols)
            log(f"  {table_name:<15} → {len(results[table_name]):,} rows returned", 1)
        else:
            log(f"  WARNING: No result set for {table_name}", 1)
...
def save_to_output_schema(conn, results, run_ts):
    ...
    for table_name, df in results.items():
        ...
        col_defs = ", ".join([f"[{c}] {_sql_type(df_out[c])}"
